...
{
    [Authorize]
    public class DanhMucTyGiaController : BaseController
    {
        ...
        {
            var data = db_.Configs.Where(a => a.parentId == appGlobal.IdDanhMucTyGia && a.xacNhanXoa == false && (string.IsNullOrEmpty(MoTaSearch) == true || a.MoTa.ToUpper().Contains(MoTaSearch.ToUpper()))).ToList();
            ViewBag.data = data;
            return PartialView();
        }
        ...
        {
            var data = db_.Configs.Where(a => a.Id == id && a.parentId == appGlobal.IdDanhMucTyGia).FirstOrDefault();
            ViewBag.data = data;
            return PartialView();
        }
        [ValidateInput(false)]
        ...
        public JsonResult _InsertFun(string MoTa, string moTaChiTiet)
        {
            ...
            try
            {
                ...
                if (MoTa == "" || MoTa == null)
                {
                    return Json(new { status = -1, title = "", text = "Vui lòng nhập tên tỷ giá", obj = "" }, JsonRequestBehavior.AllowGet);

                }
                var model_copy = new Config();
                model_copy.parentId = appGlobal.IdDanhMucTyGia;
                model_copy.code = "ExchangeRate";
        ...
        public JsonResult _EditFun(int id, string MoTa, string moTaChiTiet)
        {
            ...
            try
            {
                ...
                var data = db_.Configs.Where(a => a.Id == id && a.xacNhanXoa == false && a.parentId == appGlobal.IdDanhMucTyGia).FirstOrDefault();
                if (data != null)
                {
                    data.MoTa = MoTa;

...
{
    [Authorize]
    public class DanhMucTinhTrangBaiVietController : BaseController
    {
        ...
        public async Task<ActionResult> GetList(string TenTinhTrangSearch)
        {
            var data = db_.Configs.Where(a=>a.parentId == appGlobal.IdTinhTrangBaiViet && a.xacNhanXoa ==false &&(string.IsNullOrEmpty(TenTinhTrangSearch) ==true || a.MoTa.ToUpper().Contains(TenTinhTrangSearch.ToUpper()))).ToList();
            ViewBag.data = data;
            return PartialView();
        }
        ...
        {
            var data = db_.Configs.Where(a=>a.Id==id && a.parentId == appGlobal.IdTinhTrangBaiViet).FirstOrDefault();
            ViewBag.data = data;
            return PartialView();
        }
        [ValidateInput(false)]
        ...
        public JsonResult _InsertFun(string MoTa,string moTaChiTiet)
        {
            ...
            try
            {
                ...
                if (MoTa == "" || MoTa == null)
                {
                    return Json(new { status = -1, title = "", text = "Vui lòng nhập tên trạng thái", obj = "" }, JsonRequestBehavior.AllowGet);
                }
                var model_copy = new Config();
                model_copy.parentId = appGlobal.IdTinhTrangBaiViet;
                model_copy.code = "Statusposts";
        ...
        public JsonResult _EditFun(int id,string MoTa,string moTaChiTiet)
        {
            ...
            try
            {
                ...
                var data = db_.Configs.Where(a=>a.Id==id && a.xacNhanXoa == false && a.parentId ==appGlobal.IdTinhTrangBaiViet).FirstOrDefault();
                if(data != null)
                {
                    data.MoTa = MoTa;

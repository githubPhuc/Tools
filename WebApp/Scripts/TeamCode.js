...
function Pagination(Id) {
    var totalItems = $(`${Id}`).val(); // Tổng số mục
...
function showPage(pageNumber, element, Id) {
    var paginationInfo = Pagination(`${Id}`)
...
function prevPage(Id, element) {
    if (currentPage > 1) {
        currentPage--;
        showPage(currentPage, element, Id);
    }
}

function nextPage(Id, element) {
    var tmp = Pagination(`${Id}`)
